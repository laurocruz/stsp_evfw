...
/*
   ...
   subsets (called "cases"). When s1_Gain23, s2, ..., s6 has been chosen, Case6 is
   used to discriminate between 7 cases. When s1_Gain23, s2, ..., s8 has been
...
GainType Gain23()
{
    Node *s2, *s3, *s4, *s5, *s6 = 0, *s7, *s8 = 0, *s1_Gain23Stop;
    ...
    if (!s1_Gain23 || s1_Gain23->Subproblem != FirstNode->Subproblem)
        s1_Gain23 = FirstNode;
    s1_Gain23Stop = s1_Gain23;
    for (X2 = 1; X2 <= 2; X2++) {
        Reversed = X2 == 1 ? OldReversed_Gain23 : (OldReversed_Gain23 ^= 1);
        do {
            s2 = SUC(s1_Gain23);
            if (FixedOrCommon(s1_Gain23, s2))
                continue;
            G0 = C(s1_Gain23, s2);
            Breadth2 = 0;
            ...
            for (Ns2 = s2->CandidateSet; (s3 = Ns2->To); Ns2++) {
                ...
                for (X4 = 1; X4 <= 2; X4++) {
                    ...
                    if (X4 == 1 && s4 != s1_Gain23 && !Forbidden(s4, s1_Gain23) &&
                        2 * SegmentSize(s2, s3) <= Dimension &&
                        (!c || G2 - c(s4, s1_Gain23) > 0) &&
                        (G3 = G2 - C(s4, s1_Gain23)) > 0) {
                        Gain =
                            BridgeGain(s1_Gain23, s2, s3, s4, 0, 0, 0, 0, 0, G3);
                        if (PenaltyGain > 0 || Gain > 0)
                            return Gain;
                    }
                    if (X4 == 2 &&
                        !Forbidden(s4, s1_Gain23) &&
                        (CurrentPenalty > 0 ||
                         TSPTW_Makespan || !c || G2 - c(s4, s1_Gain23) > 0)) {
                        Gain = G2 - C(s4, s1_Gain23);
                        if (CurrentPenalty > 0 ||
                            TSPTW_Makespan || Gain > 0) {
                            Swap1(s1_Gain23, s2, s3);
                            if (Improvement(&Gain, s1_Gain23, s2))
                                return Gain;
                        }
                    }
                    if (G2 - s4->Cost <= 0)
                    ...
                    for (Ns4 = s4->CandidateSet; (s5 = Ns4->To); Ns4++) {
                        ...
                        for (X6 = 1; X6 <= 2; X6++) {
                            ...
                            G4 = G3 + C(s5, s6);
                            Gain6 = 0;
                            if (!Forbidden(s6, s1_Gain23) &&
                                (CurrentPenalty > 0 ||
                                 TSPTW_Makespan ||
                                 !c || G4 - c(s6, s1_Gain23) > 0) &&
                                (Gain6 = G4 - C(s6, s1_Gain23)) > 0) {
                                if (CurrentPenalty > 0 ||
                                    TSPTW_Makespan || Gain6 > 0) {
                                    if (Case6 <= 2 || Case6 == 5 ||
                                        Case6 == 6) {
                                        Make3OptMove(s1_Gain23, s2, s3, s4, s5,
                                                     s6, Case6);
                                        if (Improvement(&Gain6, s1_Gain23, s2))
                                            return Gain6;
                                    }
                                }
                                Gain =
                                    BridgeGain(s1_Gain23, s2, s3, s4, s5, s6, 0,
                                               0, Case6, Gain6);
                                if (PenaltyGain > 0 || Gain > 0)
                                    return Gain;
                            }
                            ...
                            for (Ns6 = s6->CandidateSet; (s7 = Ns6->To);
                                ...
                                for (X8 = 1; X8 <= 2; X8++) {
                                    ...
                                    if (FixedOrCommon(s7, s8)
                                        || Forbidden(s8, s1_Gain23))
                                        continue;
                                    G6 = G5 + C(s7, s8);
                                    if (CurrentPenalty > 0 ||
                                        TSPTW_Makespan ||
                                        !c || G2 - c(s8, s1_Gain23) > 0) {
                                        Gain = G6 - C(s8, s1_Gain23);
                                        if (CurrentPenalty > 0 ||
                                            TSPTW_Makespan || Gain > 0) {
                                            if (Case8 <= 15) {
                                                Make4OptMove(s1_Gain23, s2, s3,
                                                             s4, s5, s6,
                                                             s7, s8,
                                                             Case8);
                                                if (Improvement
        ...
        while ((s1_Gain23 = s2) != s1_Gain23Stop);
    }
    return 0;
}
...
/*
    ...
    contains the edges (s1_Gain23,s2) and (s4,s3), in that order. A (*) signifies
